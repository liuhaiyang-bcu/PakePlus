...
document.addEventListener('DOMContentLoaded', function() {
    // 为输入框和文本域添加needsclick类，避免FastClick干扰
    function addNeedsclickToInputs() {
    ...
    // 初始添加
    addNeedsclickToInputs();
    
    // 为动态创建的输入框也添加needsclick类
    const observer = new MutationObserver(function(mutations) {
        ...
        if (needsUpdate) {
            ...
            setTimeout(addNeedsclickToInputs, 0);
        }
    });
    
    observer.observe(document.body, { childList: true, subtree: true });
    
    // 延迟初始化FastClick，确保所有输入框都已处理
    setTimeout(() => {
...
// Pointer → Touch 事件适配（用于仅支持 PointerEvent 的触摸屏，如 Windows 触屏）
;(function(){
    try {
        // 若浏览器支持 PointerEvent，但不支持 TouchEvent API，则做适配
        var supportsPointer = !!window.PointerEvent;
        var supportsTouchEventAPI = 'ontouchstart' in window;
        if (supportsPointer && !supportsTouchEventAPI) {
            var activePointers = new Map();

            function toTouchList(pointerEvent){
                var touches = [];
                // 仅保留当前指针（单指场景足够覆盖本项目现有用法）
                touches.push({
                    identifier: pointerEvent.pointerId || 0,
            ...
            function dispatchSyntheticTouch(type, originalEvent){
                var target = originalEvent.target || document;
                var evt = new Event(type, { bubbles: true, cancelable: true });
                var touches = toTouchList(originalEvent);
                // 注入最常用的 TouchEvent 字段（满足现有代码 e.touches[0] 访问）
                evt.touches = (type === 'touchend' || type === 'touchcancel') ? [] : touches;
                evt.changedTouches = touches;
                evt.targetTouches = evt.touches;
                // 传递原事件引用
                evt._originalPointerEvent = originalEvent;
            ...
            document.addEventListener('pointermove', function(e){
                if (e.pointerType === 'touch' && activePointers.has(e.pointerId)) {
                    dispatchSyntheticTouch('touchmove', e);
                }
            }, true);

            // pointerup/cancel → touchend/touchcancel
            ...
            document.addEventListener('pointercancel', function(e){
                if (e.pointerType === 'touch' && activePointers.has(e.pointerId)) {
                    dispatchSyntheticTouch('touchcancel', e);
                    activePointers.delete(e.pointerId);
                }
            }, true);
        }

...
function renderHoroscopeCard(container, data) {
  ...
  card.innerHTML = `
    ...
    <div class="horoscope-footer">数据来源：xxapi.cn<br>相关内容来自网络，相关数据仅供参考</div>
  `;
...
function showHoroscopeDetailModal(d, icon, todayStr) {
  ...
  function getDetailText() {
    return [
      ...
      `🔔⚠️ 本日需注意：${d.shortcomment || ''}`,
      `🌟⭐️ 综合运势：${d.fortunetext?.all || ''}  (${parseInt(d.index?.all)||0}%)`,
      `💪🩺 健康：${d.fortunetext?.health || ''}  (${parseInt(d.index?.health)||0}%)`,
      `💖💕 爱情：${d.fortunetext?.love || ''}  (${parseInt(d.index?.love)||0}%)`,
      `💰🪙 财运：${d.fortunetext?.money || ''}  (${parseInt(d.index?.money)||0}%)`,
      `💼📈 工作：${d.fortunetext?.work || ''}  (${parseInt(d.index?.work)||0}%)`,
      `🍀🎲 幸运数字：${d.luckynumber || '-'}`,
    ...
    <div class="modal-content horoscope-detail-modal-content" style="max-width:440px;background:${bg};border-radius:18px;box-shadow:0 8px 32px #0003;padding:0;overflow:hidden;">
      ...
      <div class="modal-body" style="padding:22px 24px 18px 24px;max-height:62vh;overflow:auto;">
        ...
        <div style="text-align:right;margin-top:18px;display:flex;gap:12px;justify-content:flex-end;">
          ...
          <button id="horoscope-share-btn" class="horoscope-detail-btn" style="font-size:1.08em;background:${btnBg};color:${btnColor};border:none;border-radius:8px;padding:8px 22px;box-shadow:0 2px 8px #fbc2eb33;cursor:pointer;">📤 分享</button>
        </div>
      </div>
    </div>
  `;
  ...
  modal.querySelector('#horoscope-share-btn').onclick = function() {
    ...
    if (window.plus && plus.share && plus.share.sendWithSystem) {
      ...
      navigator.share({title: d.title + ' ' + todayStr + '运势', text: text});
